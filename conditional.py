...
class Encoder(nn.Module):
    def __init__(self, d_input, d_hidden, d_latent, n_classes=10):
        super().__init__()
        self.embed = Linear(d_input, d_hidden)
        self.y_embed = nn.Embedding(n_classes, d_hidden)
    ...
    def forward(self, x, y):
        h = self.embed(x)
        h = h + self.y_embed(y)      # <-- CONDITIONING
        h = self.block1(h)

        mu = self.mu(h)
        logvar = self.logvar(h).clamp(-6, 2)
        return mu, logvar
...
class Decoder(nn.Module):
    def __init__(self, d_latent, d_hidden, d_output, n_classes=10):
        super().__init__()
        self.y_embed = nn.Embedding(n_classes, d_latent)
    ...
    def forward(self, z, y):
        z = z + self.y_embed(y)      # <-- CONDITIONING
        return self.net(z)
...
def train(model, loader, device, epochs=200):
    ...
    os.makedirs("conditional_samples", exist_ok=True)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0:
            ...
            with torch.no_grad():
                ...
                save_image(x, f"conditional_samples/epoch_{epoch:03d}.png", nrow=3)
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "conditional_vae.pt")