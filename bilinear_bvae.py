...
def elbo_loss(x, logits, mu, logvar, beta=1.0, C=0.0):
...
def train(model, loader, device, epochs=200):
    ...
    os.makedirs("bvae_encoder_samples", exist_ok=True)
    ...
    for epoch in range(epochs):
        ...
        C = min(C_max, C_max * epoch / C_warmup)
        ...
        if epoch % 5 == 0:
            ...
            with torch.no_grad():
                ...
                save_image(
                    x,
                    f"bvae_encoder_samples/epoch_{epoch:03d}.png",
                    nrow=3
                )
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "bvae_encoder_vae.pt")